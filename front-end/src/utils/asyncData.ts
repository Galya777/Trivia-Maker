...
export class AsyncDataSer<T> {
    public error = false;
    public errorsData?: Error[] = undefined;
     ...
     * This is useful to substitute a plain object on a AsyncDataSer instance (received from server for example)
     ...
     * @param data
     */
    public static errored<T>(errors?: any[], data: T | null = null): AsyncDataSer<T> {
    ...
    public static filterForData<T>(input: Observable<AsyncDataSer<T>>, orError: boolean): Observable<AsyncDataSer<T>> {
        return input.pipe(
            filter(adata => adata && AsyncDataSer.hasData(adata, orError))
        );
    }
}
